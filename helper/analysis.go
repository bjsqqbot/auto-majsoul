...
func analysisPlayerWithRisk(playerInfo *model.PlayerInfo, mixedRiskTable riskTable) error {
	...
	// 计算手牌
	fmt.Println("舍张 ", playerInfo.DiscardTiles)
	fmt.Println("手牌 ", util.Tiles34ToTiles(playerInfo.HandTiles34))
	t := -1
	for i := 0; i < 34; i++ {
		if playerInfo.HandTiles34[i]-pretile[i] == 1 {
			t = i
			break
		}
	}
	if t == -1 { // 和上一轮的牌一样，摸到的牌是最后的舍张
