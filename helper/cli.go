...
func printResults14WithRisk(results14 util.Hand14AnalysisResultList, mixedRiskTable riskTable) (int, bool) {
	if len(results14) == 0 {
		return -1, false
	}
	...
	for _, result := range results14 {
		...
		if r.result13.Shanten == 0 {
			reach = true
		}
		if bestCard == -1 && r.highlightAvgImproveWaitsCount {
			bestCard = r.discardTile34
		}
		r.printWaitsWithImproves13_oneRow()
	}
	return bestCard, reach
}
